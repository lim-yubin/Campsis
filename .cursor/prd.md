...
- 저장 위치: ~~`~/Library/Application Support/<app>/`~~ → **`~/Documents/Campsis/`** (2026-09-03 변경, D54 참조). 사용자가 파인더로 원본·정리본·스크린샷을 직접 열람하고, 샌드박스 앱(미리보기)이 원본을 제자리 편집·저장할 수 있게 하기 위함. 임베딩 모델(`Models/`)만 대용량 내부 에셋이라 구 위치(`Application Support/Campsis/Models`)에 유지.
- **경로 결정을 코드 전체에 흩뿌리지 않고 한 곳에서 관리한다** (`AppPaths.dataRoot`)
...
| D40  | ~~MD 생성은 백그라운드/지연 처리~~ **개정(2026-09-01, D48)**: Luna 단일 파이프라인 | ~~캡처 즉시 로컬 OCR 텍스트로 저장·임베딩해 검색은 바로 가능. Luna MD 생성은 온라인 시 보강.~~ **개정:** always-online 전제로 즉시 로컬 검색(OCR) 불필요 → 캡처 후 Luna 1회 호출로 이해(OCR+요약+태그+MD)를 완료하고 그 MD로 임베딩. 캡처 직후 짧은 Luna 왕복 지연 발생(수용). provider=local이면 pending 유지 |
| D41  | ~~OCR을 프로토콜로 추상화 (Windows 확장 대비)~~ **개정(2026-09-01, D48)**: OCR 제거 | ~~`OCRProcessing` 프로토콜 + AppleVisionOCR(macOS 로컬 기본).~~ **개정:** AppleVision OCR(및 `OCRProcessing` 프로토콜)을 완전히 제거하고 `import Vision` 의존성 삭제. 이미지 OCR·이해는 `LunaMarkdownGenerator`가 비전 입력(base64 data URL)으로 단일 호출 처리. Windows 확장 시에도 동일 Luna 경로 재사용(크로스플랫폼) |
...
| D48  | Luna 단일 이해 파이프라인 (offline/no-key 미지원)     | 사용자 결정(2026-09-01). 서비스는 항상 Luna 사용(제작자 키 구독 또는 BYOK)을 전제 → 오프라인·키 없음 상황은 미지원. 이에 따라 AppleVision OCR과 로컬 요약/태그 생성(TextGenerator.analyze)을 제거하고, 캡처된 모든 소스를 Luna 비전+MD 단일 호출로 이해(OCR+요약+태그+MD)한 뒤 그 MD를 bge-m3로 임베딩. ~~로컬 채팅용 MLX 프리로드는 당분간 유지~~ → D49로 제거. D40·D41을 개정 |
| D49  | 로컬 생성 LLM(MLX/Qwen·Apple FM) 전면 제거 → 채팅도 Luna 단일 | 사용자 결정(2026-09-01, D48 후속). 로컬 생성 모델은 품질 한계·소비자 하드웨어(RAM/용량) 부담·앱 용량 증가가 커서 제거. 채팅/생성은 GPT-5.6 Luna(클라우드)로 일원화. 삭제 대상: `MLXGenerator`·`AppleFMGenerator`·`TextGenerator`·`AppleFMChatEngine`·`MLXChatEngine`, MLX/HuggingFace SPM 의존성(mlx-swift-lm·swift-huggingface). **유지**: 로컬 임베딩(CoreML bge-m3) 및 그 토크나이저용 swift-transformers, 검색·저장·MD 진실원. `AIProvider`는 `.luna` 단일(더 이상 엔진 선택 없음), 키 미설정 시 채팅은 설정 안내 메시지 반환. `modelStatus`는 임베딩 모델 로드 상태를 표시하도록 재정의. 효과: 앱 번들·의존성·메모리 사용 대폭 감소. 트레이드오프: 채팅은 API 키(구독/BYOK) 필수(always-online 전제와 일치). Phase 3.5는 이력 보존 처리 |
| D50  | UX 1차 개선 배치 + 후속 정리 (사용자 실사용 피드백) | 사용자 결정(2026-09-01). **적용:** 인스펙터 이미지 미리보기(A1)·폭 자유조절(A2)·전역 토글(A3)·메모리 탭 확대(A4), 정리본 복사 2종(마크다운/일반, B2), 채팅 Enter=전송·Shift+Enter=줄바꿈(C1, 커스텀 `ChatTextEditor`), 진입 시 최하단 스크롤(C2). **후속 확정 스코프:** ~~B1(인스펙터 메모리→상세 전환을 메모리 메뉴 UX로 통일)~~ **→ 스킵(2026-09-02): 시제품 구현 후 불필요 판단, 되돌림. 인스펙터는 채팅 출처 미리보기+메모리 목록 읽기전용 스왑 현행 유지**, B3(원본 편집→정리본 재생성+재벡터화, **텍스트/노트/파일 텍스트 한정**, 이미지 제외), B5(메모리 리스트 타이틀 명확화). **B5 적용(2026-09-02):** Luna 생성 제목을 `source.title` 컬럼(v6)에 저장하고 공용 `Source.displayTitle`(제목→요약→창제목→본문 첫 줄→유형명)로 리스트·인스펙터·출처·상세 제목을 통일. **B3 적용(2026-09-02):** 텍스트/노트/파일 원본을 상세 '원본' 탭에서 편집→`ProcessingQueue.regenerate`로 정리본 재생성+재임베딩(이미지/음성 제외). 정리본 직접 수정 기능은 유지(용도 구분), `markdown_edited` 컬럼(v7)로 수동 편집 추적해 재생성이 수동 편집본을 덮어쓸 때 확인 alert. **D1 적용(2026-09-01):** 커스텀 `MarkdownTextView`에 GFM 파이프 표(`NSTextTable`), 클릭 가능한 링크(`.link`), 로컬 이미지 인라인 첨부(`NSTextAttachment`, 원격 http는 대체 텍스트) 추가 — 채팅 답변·정리본 미리보기·상세에 공통 적용. 채팅 기억 범위는 현행 유지(동일 대화 내 최근 6메시지 컨텍스트, `historyLimit`). **사이드바 재설계(2026-09-02):** UI/UX 재설계 캔버스 방향을 반영해 사이드바를 상단 검색창 + `라이브러리`(전체 기억·오늘·텍스트·스크린샷·메모·음성·파일) + `대화`(헤더 + 버튼)로 재구성. 필터 소유권을 `MemoriesView` 내부 드롭다운에서 사이드바로 이전(`SidebarSection.library(LibraryItem)`), 사이드바 검색은 전역 검색으로 동작. 항목별 개수 표시는 후속 과제로 보류. **사이드바 상단 툴바/너비(2026-09-02):** 새 채팅 버튼을 상단 아이콘 묶음(`[토글][작성]`)으로 이동, 시스템 사이드바 토글을 커스텀 토글(`columnVisibility`)로 대체(순서·간격·호버 제어), 접힘 시에도 동일 묶음 유지. 사이드바 너비는 `navigationSplitViewColumnWidth`가 강제되지 않아 하부 `NSSplitViewItem` min/max 두께(240~340)를 `NSViewRepresentable`로 직접 강제(접기/펼치기 시 지연 재적용으로 리셋 보정), 메인 최소 너비 520 |
| D53  | 메모 + LLM 위키 + MCP (Phase 8, 비개발자용 LLM wiki) | 사용자 결정(2026-09-02, 설계 확정·미구현). Karpathy "LLM wiki" 패턴(raw→요약이 아니라 **누적 종합 위키**를 LLM이 유지)을 **비개발자(직장인·학생·사무직) 대상**으로 재해석. **레이어:** ①메모(정리본, raw→MD 자동 생성·항상 검색) ②위키(토픽 허브=소속 메모들을 **종합한** 문서, 폴더 아님) ③채팅. **승격 모델:** 캡처 핫패스엔 선택 없음(무마찰). 메모함에서 "위키 미주입/주입됨" 구분 → 사용자가 **체크박스 배치 선택** → "위키에 정리" → **라우팅 미리보기**(기존 위키 임베딩 매칭 우선, 없으면 새 위키 후보; 멀티토픽→멀티위키 가능, 상위만 미리 선택) → "정리 실행". **재합성(D.2):** 실행 시 즉시 소속 등록+백링크(D.1), 이어 **위키별 1회 증분 재합성**([현재 위키 페이지 + 새 메모들]만 Luna 전달, 전체 재독 금지→선형 비용), 백그라운드 "정리 중" 상태로 비차단. 명시적 배치 트리거라 비용 통제됨(자동·건건 재합성 금지). `reasoning_effort: low`. **채팅:** 위키 우선 검색→부족 시 메모 드릴다운(중복·비용 방지), 출처를 위키/메모로 구분 배지. **검색:** 사이드바 의미검색(Phase 6) 대상에 위키 페이지 포함. **MCP:** 위키만 read(정제·고신호·프라이버시), 신선도(Lint) + 권한 스코핑 전제. **데이터 모델:** `Wiki`, `NoteWikiLink`(다대다), `WikiWikiLink`(관련 위키 백링크); 위키 MD도 파일 진실원(D39). **필수 인프라:** 위키 유지보수(Lint — 중복 위키 병합·모순·고아 감지), 사람 편집 위키 보호(`markdownEdited` 확장). **리스크:** 토픽 폭발·중복, 재합성 환각, 관리자 부재 위키 부패, 대규모 비용(목적지 위키 수 상한). **결정 이탈 검토:** D.2를 지연/온디맨드로 미루는 안도 논의했으나, 승격이 명시적 배치라 **버튼 클릭 즉시 재합성**으로 확정. 기존 **D51(E1 문서연결·E2 마인드맵)** 을 "note↔topic-hub + 그 백링크 그래프"로 구체화. **구현 상세 스펙은 `docs/phase8-llm-wiki.md`**(데이터 모델·라우팅·재합성 프롬프트·엣지 케이스·미결 질문 OW1~OW6), 요약은 ROADMAP Phase 8 참조. **OW1 확정(2026-09-02): 임계값 기반 하이브리드** — 순수 반통제형(B) 폐기(데이터 성격을 알 수 없어 강제 편입 시 주제 쏠림·왜곡). 정리본↔기존 위키 유사도(T_match)로 분기: ≥면 편입 제안, 중간이면 후보 선택, 미만이면 강제 편입 없이 새 위키 제안(C.3 명시 승인). 콜드스타트엔 T_match↑로 새 위키 유도→성숙 시↓, 과다 집중 위키는 Lint가 분할 제안. **OW3 확정(2026-09-02):** 매칭 신호=`0.7·코사인+0.3·태그겹침`, 2단 밴드 T_high 0.55/T_low 0.40(사이=사용자 선택, 미만=새 위키), 콜드스타트 T_high 0.65→0.55 스케일, T_merge 0.80(병합), N_max 3(메모당 목적지). 현행 코사인 스케일(관련성 0.4·검색 0.3) 기준 초기값, `match_score` 로그로 사후 튜닝. **OW2 확정(2026-09-02):** 위키 대표 임베딩=**위키 페이지 임베딩**(centroid 아님) — 채팅·MCP가 어차피 요구해 인프라 0·유지 단일화, 위키=종합 문서라 의미상 정확, centroid는 멀티토픽 drift. 폴백: 재합성 전 위키만 centroid 임시. 스키마: `embedding`에 `kind`(note/wiki) 추가. **OW6 확정(2026-09-02):** 자동 추천 강도 = 배지만 표시(자동 체크 ✗) + 옵트인 넛지. 경계 원칙 "무엇을 올릴지=사용자, 어디로 갈지=시스템(C.3)". 메모 목록은 T_high 이상만 추천 배지·프리셀렉트 없음, 사장 위험은 "정리 안 된 메모 N개·추천 묶음 보기" 옵트인 배너로 완화(자동 커밋 없음), off 가능. **OW4 확정(2026-09-02):** 되돌리기=**MD 스냅샷**(git 유사 이력 아님). 위키 MD 쓰기 직전 이전 버전을 `wiki_revision` 테이블+`wiki_revisions/{wikiId}/{id}.md`로 보관(위키당 최근 10), 되돌리기=직전 MD 복원+재임베딩(승격이면 added_source_ids 링크 제거+member_count 복구), 완료 토스트 원클릭. **OW5 확정(2026-09-02):** Lint 트리거=이벤트 기반 국소(위키 메뉴 진입·승격/재합성 직후)+저빈도 전수(앱 유휴 하루 1회, 변경 있을 때). Lint 자체 LLM 비호출(중복=벡터·고아=DB·stale=날짜 로컬, 모순만 재합성 piggyback). 비파괴 제안 카드+dismiss 쿨다운, MVP는 이벤트 기반 우선. **→ OW1~OW6 전부 확정, Phase 8 착수 준비 완료** |
| D52  | 핵심 UX/UI 개선 6종 배치 (Campsis Core UX) | 사용자 결정(2026-09-02). 계획 `campsis_core_ux_improvements`를 6개 Phase로 구현. **P1 열기 인터랙션 통일:** 단일클릭=미리보기(읽기전용 인스펙터), 미리보기 헤더의 `전체 보기 →`=편집 가능한 상세로 통일. 인스펙터 `SourcePreviewView`에 `전체 보기`/`원본`/`원문 열기` 승격 액션 추가, 상세는 시트로 연결(`SourceDetailView(initialTab:)`). 채팅 출처·인스펙터 목록에 현재 미리보기 항목 하이라이트. **P2 정리본↔원본:** 대등 세그먼트 대신 정리본 주 화면 + `원본 보기 (N자)` 보조 토글, 편집 모델 라벨 명확화(원본수정→재생성=주 경로, 정리본 직접수정=고급/표현 다듬기). **P3 채팅 출처 칩:** 접힘형 `출처 N개`를 항상 보이는 컴팩트 칩 행으로(관련도순, `+N개 더보기`). **P4 리스트 스캔성:** `MemoryRowView`에 스니펫+주제 미니칩+썸네일+선택 하이라이트. **P5 복사 일관화:** 섹션별 복사 버튼 + 공용 `copyToast` modifier(상세·미리보기; 채팅은 기존 인라인 체크마크 유지). **P6 의미검색:** 사이드바 검색을 `VectorSearchEngine`로 승격(`AppState.searchEngineRef` 노출, 350ms 디바운스, 텍스트 정확일치+의미유사 병합, minScore 0.25) + `이 검색어로 채팅하기` 브리지(`pendingChatPrefill`). **결정 이탈:** 계획의 SourceViewer(.preview/.full) 완전 통합은 `SourceDetailView` 편집 로직 재작성 위험이 커, 미리보기→상세 시트 브리지 + 선택적 초기 탭 파라미터로 대체(상세 편집 단일 소스 유지). 파일 간 변경이 얽혀 Phase별 독립 커밋 대신 단일 커밋으로 반영 |
| D51  | 지식베이스 확장 기능(문서 연결·마인드맵)은 별도 Phase로 보류 | 사용자 결정(2026-09-01, prd_only). **E1 문서(기억) 간 연결**: LLM 위키처럼 기억끼리 관련 문서를 연결. 자동(임베딩 유사도/LLM 추론) vs 수동 링크 방식은 설계 시 결정. **E2 마인드맵 UI**: 옵시디언 그래프 뷰 스타일 시각화 — E1의 링크 데이터가 선행 조건. 두 기능 모두 지금은 구현하지 않고 향후 Phase(핵심 출시 이후)로 편성. 구현 착수 전 별도 설계 문서 필요 |
| D54  | 데이터 저장 루트 이전(`Application Support` → `~/Documents/Campsis`) + 원본 외부 편집 재생성 | 사용자 결정(2026-09-03). **동기:** 스크린샷 원본을 미리보기로 편집하면 복제본만 생기고 원본 제자리 저장이 안 되는 문제(샌드박스 앱은 `~/Library/Application Support` 하위에 쓰기 불가; 격리 속성 `com.apple.quarantine` 제거만으로는 불충분 — **위치 제약**). 추가로 사용자가 앱을 거치지 않고 원본·정리본·스크린샷을 직접 열람하고 싶어함. **결정:** 데이터 루트를 `~/Documents/Campsis`로 이전(§19.6). `AppPaths.dataRoot` 단일 베이스 + 일회성 `migrateToDocumentsIfNeeded()`(구 위치→신 위치 이동, `campsis.db`+wal/shm·미디어·마크다운·위키 폴더; 대용량 내부 에셋 `Models/`만 구 위치 유지; 멱등; `AppDatabase.makeDefault()` 최상단 DB 오픈 이전 실행). DB 상대경로는 하위 구조 유지로 무변경. **원본 편집→재생성:** 정리본 OCR/이해가 Luna 비전으로 디스크 파일을 읽는 점을 활용, 원본(스크린샷/이미지/PDF/txt)을 외부 앱에서 수정→앱 복귀 시 mtime 비교(`EditWatcher`)로 감지→확인 후 `ProcessingQueue.regenerateFromDisk`(이미지=디스크 재읽기, 문서=`FileTextExtractor`로 content 재추출)로 정리본 재생성+재임베딩. **이로써 D50/B3의 "이미지 제외" 제약이 완화됨**(이미지도 외부 편집 경로로 재생성 가능). **주의/트레이드오프:** 비샌드박스 앱의 첫 `~/Documents` 접근 시 macOS 권한 프롬프트 가능, Documents가 iCloud 동기화 대상이면 캡처 데이터가 iCloud로 동기화될 수 있음. 캡처 해상도 버그(스케일1 디스플레이 검은 여백)도 함께 수정(`CGDisplayCopyDisplayMode` 픽셀 해상도 사용) |
